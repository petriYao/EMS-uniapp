...
import { lookSalesReturn } from '@/api/modules/transferOrder'
import { executeBillQueryApi, saveApi } from '@/api/commonHttp'

//销售退货-扫描单号
export const getSalesCase = async (searchValue: any) => {
    ...
    for (const item of TreeEntity) {
      ...
      const data = {
        currentList: [
          ...
          {
            label: '客户',
            value: res.data.Result.Result.CustId?.Name[0].Value,
            disabled: true,
            type: 'input',
            style: { width: '65%' }
          },
...
// 销售退货-扫描条码
export const salesScanBarcode = async (searchValue: any) => {
