  ...
  if (res && res.data) {
    ...
    for (const item of TreeEntity) {
      ...
      if (lowerRes && lowerRes.data && lowerRes.data.length > 0) {
        for (const item of lowerRes.data) {
          inventory += item[0]
        }
      }
      //查储位
      ...
      const data = {
        ...
        //源单单号
        SourceOrderNo: item.MoBillNo,
        //源单行号
        SourceOrderLineNo: item?.MoEntrySeq,
        //批号
