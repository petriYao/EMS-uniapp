...
export const getSimple = async (searchValue: any) => {
  ...
  if (res && res.data) {
    ...
    for (const item of tempDataList) {
      const key = `${item.SrcBillNo}-${item.SrcEntrySeq}-${item.Lot}-${item.MaterialCode}`
      if (keyMap.has(key)) {
        ...
        // 拼接字符串字段
        // 拼接 stockLocName
        const stockLocSet = new Set(existing.detailList.stockLocName.split(','))
        ...
        // 拼接 storageLocation
        const storageSet = new Set(existing.detailList.storageLocation.split(','))
...
*/
    return { dataList: tempDataList, fid }
  }
}
