...
onBeforeMount(() => {
  ...
  switch (reactiveData.title) {
    case '生产领料':
      if (UserAuthority) {
        if (UserAuthority.includes('37-1')) {
          //生产领料-单码双扫
          reactiveData.radioList[0].disabled = false
        }

        if (UserAuthority.includes('37-2')) {
          //生产领料-扫单领料
    ...
    case '简单生产领料':
      if (UserAuthority) {
        if (UserAuthority.includes('38-1')) {
          //扫码入库
          reactiveData.radioList[0].disabled = false
        }

        if (UserAuthority.includes('38-2')) {
          //扫单入库
          reactiveData.radioList[1].disabled = false
        }
      }
    ...
    case '委外领料':
      if (UserAuthority) {
        if (UserAuthority.includes('39-1')) {
          //扫码入库
          reactiveData.radioList[0].disabled = false
        }

        if (UserAuthority.includes('39-2')) {
          //扫单入库
          reactiveData.radioList[1].disabled = false
        }
      }
