  ...
  if (reactiveData.detailsList.isFE) {
    if (reactiveData.lowerCamelCaseList.isInteger) {
      ...
      if (reactiveData.Model.FEntity.length === 0) {
        ...
        const saveRes = await saveSalesOrder(reactiveData.Model, false)
    ...
    if (reactiveData.Model.FEntity.length === 0) {
      const saveRes = await deleteSalesOrder(reactiveData.Model.FID)
...
<template>
  ...
  <u-loading-page
    ...
    loadingText="保存中"
    :loading="reactiveData.isShow == false"
  />
