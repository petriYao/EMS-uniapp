...
import { dateTimeStr } from '@/common/getdateTime.js'
...
//条码数据单查询
export function GetBarcodeData(code) {
  //0 FID
...
//生产工单查询
export function GetBarcodeDataForSCGD(getcode) {
  let data = {
    data: {
...
//所有工序
export function GetProductionGxAll(gxno) {
  let data = {
    data: {
      FormId: `${FromData.GXFromID}`,
      FieldKeys: 'FID,FNumber,FName',
...
//查询工序编号
export function GetProductionFNumber(FID) {
  let data = {
    data: {
      FormId: `${FromData.GXFromID}`,
...
//下推
export function PushProduction(EntryIds) {
  let data = {
    formid: `${FromData.SCGDFromID}`,
    data: {
      Ids: '',
...
//删除出现错误无法保存的生产汇报单
export function DeleteSCHBDProject(Ids) {
  let data = {
    formid: `${FromData.SCHBDFromID}`,
    data: {
      CreateOrgId: 0,
...
//保存更新
export function SaveProduction(canshu, Resultid) {
  let data = {
    formid: `${FromData.SCHBDFromID}`,
    data: {
      NeedUpDateFields: [],
...
//保存扫码记录
export function SaveDoNothing(sqlModel, SCGDNumber) {
  let data = {
    formid: 'QADV_SMJLBYD',
    data: {
      NeedUpDateFields: [],
      ...
      Model: {
        FID: sqlModel.Fid,
        F_WOMW_SCGDBHfFID: sqlModel.F_WOMW_SCGDBHfFID, //F_WOMW_SCGDBHfFID 生产工单编号FID内码
