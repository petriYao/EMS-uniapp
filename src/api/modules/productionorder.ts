...
//单据类型
export function DocumentType(data:any) {
	console.log("单据类型",data)
	return http({
...
//查询单据对应的部门
export function QueryAssociation(data:any) {
	console.log("查询单据对应的部门",data)
	return http({
...
//数据状态颜色返回
export function FDocumentStatusColor(state:any) { 
	console.log("数据状态颜色返回",state)
	let colorstr = ""
	switch (state) {
		case "暂存":
			colorstr = "#808080"
